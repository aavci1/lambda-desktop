...
- **SVC-3 — logind client.** Basic slice started: `lambda::system::LogindClient` connects through `lambda::dbus`, calls `Suspend`/`Hibernate`/`PowerOff`/`Reboot`, can ask the current session to `Lock` or `Terminate`, takes fd-based `Inhibit` locks, resolves session object paths through `GetSessionByPID`, subscribes to manager `PrepareForSleep`, and watches explicit or current-session `Lock`/`Unlock` signals. Shell command-launcher search now exposes lock, log out, suspend, hibernate, restart, and power-off actions through the client. `LogindTests.cpp` covers the power/session calls, inhibitor fd plumbing, current-session discovery, sleep signal, and session lock/unlock signals against a fake `org.freedesktop.login1` on a private bus, and `ShellModelTests.cpp` covers launcher action discovery. Remaining finer details: expand the full Shell power/session menu with a dedicated visible surface, confirmations, and error states, wire the client into WM idle/DPMS behavior and the lock app, hold the real delay inhibitors for `handle-power-key`, `handle-lid-switch`, and `handle-suspend-key`, honor `IdleAction` together with WM-13, and validate against a real system bus. Acceptance: closing the lid suspends (and locks first); the power key triggers the Shell power menu; `loginctl lock-session` locks; resume-from-suspend shows the lock screen.
