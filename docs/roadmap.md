...
| Shell | App registry, dock, IPC, config, icons, status fallback, and a basic live notification banner with timeout/preview/action policy exist; full notification center, clipboard, tray, and most provider control remain, gated on System Services. |
