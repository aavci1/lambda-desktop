...
- **SVC-6 — Notifications daemon.** Basic slice started: `lambda-notifications` is a Linux session-bus service with installed D-Bus activation metadata; it exports `org.freedesktop.Notifications` at `/org/freedesktop/Notifications`, implements `Notify`, `CloseNotification`, `GetCapabilities`, and `GetServerInformation`, stores an in-memory history with replacement support, parses actions, emits `NotificationClosed` and `ActionInvoked`, and emits a Lambda-private `NotificationPosted` signal for Shell observers. Shell now consumes that signal through a session-bus event pump, upserts notifications by daemon id, applies enabled/DND/history-limit policy, shows a basic top-right live banner, honors banner timeout and preview-visibility config, renders up to two action buttons, routes action clicks through a private daemon `InvokeAction` method so the daemon emits `ActionInvoked`, routes banner dismissals to `CloseNotification`, and hides visible banner state when `NotificationClosed` arrives. The SVC-4 portal Notification backend now routes `AddNotification`, replacement, close, and action relay through this daemon. `NotificationsTests.cpp` covers the spec methods/signals, Shell-facing posted signal, and private action invoke path against a private bus; Shell model tests cover daemon-id upsert, action replacement, and banner-timeout policy; `PortalNotificationTests.cpp` covers portal routing; and a process-level private-bus smoke verifies the built daemon accepts a real `Notify` call through `gdbus`. Remaining finer details: full notification-center UI, richer action layout, service-side timeout expiry, persistence/history grouping, and icon/image hints. Acceptance: `notify-send "x"` shows a banner; actions invoke; DND suppresses; history shows past notifications.
