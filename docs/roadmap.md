...
**Last updated:** 2026-06-14 (started SVC-1 D-Bus capability, SVC-2 libseat KMS device access, SVC-3 logind client basics with active-session discovery, SVC-4 portal Settings backend basics, and SVC-6 notification service basics; integrated daily-driver gap analysis: new System Services workstream SVC-*, compositor protocol/power gates WM-11…WM-21, missing-app specs, reprioritized gate)
...
This layer is mostly greenfield. A basic Linux `lambda::dbus` capability is now in tree, the KMS backend now tries libseat-managed DRM/input opens before falling back to direct opens, `lambda-portal` covers the portal Settings backend, and `lambda-notifications` covers the first Freedesktop notification service slice. There is still no FileChooser/Screencast portal backend, tray host, polkit agent, or NetworkManager/BlueZ/UPower/MPRIS/secrets/udisks integration. The Shell's existing provider notes already name these D-Bus services as consumers — this section provides the shared capability and service implementations they depend on.
...
- **SVC-1 — D-Bus capability.** Basic slice started: `lambda::dbus` opens session/system/custom-address buses through `sd-bus`, exposes sync method calls with basic-value arguments including Unix fds, object paths, string arrays, RGB tuples, empty `a{sv}` hints dictionaries, variant replies, namespaced portal Settings dictionaries, signal matching, simple property get/set, object export with method/property handlers, signal emission, fd/events/process/flush helpers, and `BusEventPump` integration with `Application::registerEventPollSource`. Focused integration tests cover `Peer.Ping`, exported method calls, Unix-fd round trips, object-path replies, exported property get/set, signal delivery, logind `GetSessionByPID`, portal Settings D-Bus shapes, and notification D-Bus shapes against a private real bus when `dbus-daemon` can run. Remaining finer details: async calls, broader generic array/dictionary/variant support beyond the currently needed service shapes, full object introspection/GetAll/PropertiesChanged helpers, additional service-client fixtures, and compositor-runtime fd pumping.
