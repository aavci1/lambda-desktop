...
- **SVC-7 — StatusNotifierWatcher + tray host.** Basic slice started: `lambda-status-notifier-watcher` is a Linux session-bus service with installed D-Bus activation metadata; it exports `org.kde.StatusNotifierWatcher` at `/StatusNotifierWatcher`, implements `RegisterStatusNotifierItem`, `RegisterStatusNotifierHost`, `RegisteredStatusNotifierItems`, `IsStatusNotifierHostRegistered`, `ProtocolVersion`, `StatusNotifierItemRegistered`, `StatusNotifierItemUnregistered`, and `StatusNotifierHostRegistered`, supports path-only item registrations by using the sender unique name, and prunes registered services on `NameOwnerChanged`. Shell now has a small `StatusNotifierWatcherClient`, registers a host, reads registered item addresses and basic `org.kde.StatusNotifierItem`/`org.freedesktop.StatusNotifierItem` metadata, watches item register/unregister and item `PropertiesChanged` events through `BusEventPump`, and renders live tray labels/glyphs in the dock status grid from category, title/id, status, icon-name, attention-icon, menu-path, and `ItemIsMenu` metadata. `StatusNotifierWatcherTests.cpp` covers properties, `GetAll`, host/item registration signals, path-only registration, owner-loss removal, client read/watch paths, SNI property reads, and item property-change refresh signals against a private bus; Shell dock tests cover the expanded status grid; and a process-level private-bus smoke verifies the built daemon answers a real `ProtocolVersion` property query through `gdbus`. Remaining finer details: icon pixmap/theme image loading, tooltip metadata, `com.canonical.dbusmenu` menus, item activation/context/scroll actions, robustness against misbehaving tray items, and real tray-app validation. Acceptance: Steam/Discord/Telegram/nm-applet-style tray icons appear and their menus work.
