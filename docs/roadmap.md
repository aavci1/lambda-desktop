...
**Last updated:** 2026-06-14 (started SVC-1 D-Bus capability, SVC-2 libseat KMS device access, SVC-3 logind client basics with active-session discovery, SVC-4 portal Settings backend basics, SVC-6 notification service basics, SVC-7 StatusNotifierWatcher basics, SVC-9 NetworkManager status basics, SVC-10 BlueZ status basics, SVC-11 UPower basics, SVC-12 MPRIS basics, and SVC-14 UDisks2 basics; integrated daily-driver gap analysis: new System Services workstream SVC-*, compositor protocol/power gates WM-11…WM-21, missing-app specs, reprioritized gate)
...
- **Phase C — connectivity and polish:** SVC-9 (NetworkManager) + Shell Wi-Fi; SVC-7 (tray host); SVC-8 (polkit); SVC-10/11/12/13/14 (BlueZ/UPower/MPRIS/secrets/udisks); SVC-15 (theming bridge); WM-15/16/17/18 (foreign-toplevel/output-management/data-control/gamma). The basic SVC-7 watcher/registry daemon, SVC-9 status client, SVC-10 status client, SVC-11 display-device battery client, SVC-12 media client, and SVC-14 removable-volume client are now in tree; Shell tray rendering, item property/menu hosting, AP/connect flows, Bluetooth pairing/connect flows, Shell now-playing/media keys, Files volume UI, and event-driven network/Bluetooth/power/media/storage updates remain.
...
- **SVC-14 — udisks2 client + Files integration.** Basic slice started: `lambda::system::UDisks2Client` connects through `lambda::dbus`, calls ObjectManager `GetManagedObjects`, reads drives and visible filesystem block devices, decodes UDisks byte-array paths and `MountPoints`, filters hidden/system volumes, formats volume names, and can call `Filesystem.Mount`, `Filesystem.Unmount`, and `Drive.Eject`. `UDisks2Tests.cpp` covers visible-volume filtering, drive/volume metadata, mount point decoding, mount/unmount calls, and eject calls against a fake `org.freedesktop.UDisks2` on a private bus. Remaining finer details: Files sidebar UI, live ObjectManager/property signal updates, encrypted volumes, job progress, mount errors, optional auto-mount policy, and real removable-media validation. Acceptance: insert a USB stick → it appears in Files → open → unmount/eject safely.
