...
| Files | Live file browser/manager exists; model layer is ahead of live UI for several commands; basic removable-media sidebar rows and mount/open/unmount/eject actions now use udisks2 (SVC-14), with unlock UI and real hardware validation still open. |
...
- SVC-14: udisks2 client — removable volumes, mount/unmount/eject, Files integration; basic client/control and Files sidebar/action slice started.
...
- **New (gap analysis):** removable-media support via SVC-14 — visible mounted/removable volume places, mount-on-open, unmount/force-unmount, eject, cancel, and error mapping are in tree; encrypted unlock UI, optional auto-mount on insert, and real hardware validation remain.

Validation: as previously specified (model, operations, trash, selection, preferences, open-with fixtures including local Preview image defaults, MIME parsing, URI-list clipboard, icon fallback, grid layout). Add: removable-volume enumerate/mount/unmount/force-unmount/eject/cancel/error mapping against SVC-14 fixtures; manual USB insert/open/eject.
