...
- **SVC-4 — xdg-desktop-portal backend.** Basic slice started: `lambda-portal` is a Linux session-bus service with installed D-Bus service metadata and `lambda.portal` selector metadata; it exports `org.freedesktop.impl.portal.Settings`, `org.freedesktop.impl.portal.AppChooser`, and `org.freedesktop.impl.portal.Notification` at `/org/freedesktop/portal/desktop`. Settings supports `Read`, `ReadAll`, `SettingChanged`, and `version`, and exposes `org.freedesktop.appearance` `color-scheme`, `accent-color`, `contrast`, and `reduced-motion`. These values now prefer Shell-owned appearance config keys editable through Settings (`appearance.color_scheme`, `appearance.accent_color`, `appearance.high_contrast`, and `appearance.reduced_motion`), with the older `LAMBDA_PORTAL_*` environment variables kept as a missing-config development fallback. AppChooser supports the OpenURI frontend's `ChooseApplication`/`UpdateChoices` path with deterministic no-UI selection: valid `last_choice` wins, otherwise the first offered app id is returned with the activation token preserved. Notification supports `AddNotification`, `RemoveNotification`, empty `SupportedOptions`, `version`, and `ActionInvoked`; it maps portal app/id pairs to SVC-6 daemon ids, forwards title/body/buttons through `org.freedesktop.Notifications.Notify`, reuses replacement ids, closes through `CloseNotification`, and relays daemon action signals back to portal clients. `PortalSettingsTests.cpp`, `PortalAppChooserTests.cpp`, and `PortalNotificationTests.cpp` cover the backends against a private bus, and a process-level private-bus smoke verifies the built `lambda-portal` service name and Settings `Read` call. Remaining finer details: validate through the real `xdg-desktop-portal` frontend with GTK/Qt apps, implement FileChooser (Open/Save -> a Lambda file-chooser surface, reusing `lambda-files` components), then Inhibit, Account, a real OpenURI chooser/default-app UI path, richer notification options, and frontend request/response handling. Acceptance: Firefox/Flatpak "Open File" shows Lambda's chooser; a Flatpak app launches and can pick files; dark-mode preference propagates to a GTK app.
