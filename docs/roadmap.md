...
**Last updated:** 2026-06-14 (started SVC-1 D-Bus capability, SVC-2 libseat KMS device access, SVC-3 logind client basics, SVC-4 portal Settings backend basics, and SVC-6 notification service basics; integrated daily-driver gap analysis: new System Services workstream SVC-*, compositor protocol/power gates WM-11…WM-21, missing-app specs, reprioritized gate)
...
- **SVC-3 — logind client.** Basic slice started: `lambda::system::LogindClient` connects through `lambda::dbus`, calls `Suspend`/`Hibernate`/`PowerOff`/`Reboot`, takes fd-based `Inhibit` locks, subscribes to manager `PrepareForSleep`, and watches session `Lock`/`Unlock` signals. `LogindTests.cpp` covers the power calls, inhibitor fd plumbing, sleep signal, and session lock/unlock signals against a fake `org.freedesktop.login1` on a private bus. Remaining finer details: wire the client into Shell power/session UI, WM idle/DPMS behavior, and the lock app; hold the real delay inhibitors for `handle-power-key`, `handle-lid-switch`, and `handle-suspend-key`; discover the current logind session path; honor `IdleAction` together with WM-13; and validate against a real system bus. Acceptance: closing the lid suspends (and locks first); the power key triggers the Shell power menu; `loginctl lock-session` locks; resume-from-suspend shows the lock screen.
