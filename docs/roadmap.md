...
- **Phase B — make the session safe and modern:** SVC-3 (logind power/inhibit) + WM-13 (idle-notify) + WM-14 (DPMS); WM-11 (session-lock) + lock app + PAM; WM-12 (screen capture) + SVC-5 (screencast portal) + SVC-4 (portal file chooser/settings); SVC-6 (notifications daemon). Basic SVC-3 client plumbing with current-session discovery plus Shell launcher and visible dock power/session actions, SVC-4 Settings backend plumbing with Shell-owned appearance config, and SVC-6 notification service plumbing are now in tree; Shell power confirmations/error states, Shell/lock/idle integration, real logind validation, Shell notification UI, and FileChooser/Screencast portal work remain.
