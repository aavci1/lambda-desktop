...
- **Phase B — make the session safe and modern:** SVC-3 (logind power/inhibit) + WM-13 (idle-notify) + WM-14 (DPMS); WM-11 (session-lock) + lock app + PAM; WM-12 (screen capture) + SVC-5 (screencast portal) + SVC-4 (portal file chooser/settings); SVC-6 (notifications daemon). Basic SVC-3 client plumbing with current-session discovery plus Shell launcher and visible dock power/session actions, SVC-4 Settings/Account/AppChooser/Inhibit/Notification backend plumbing with Shell-owned appearance config and SVC-6 routing, and SVC-6 notification service plus basic Shell live-banner plumbing are now in tree; Shell power confirmations/error states, Shell/lock/idle integration, real logind validation, full Shell notification center/actions, and FileChooser/Screencast portal work remain.
...
- **SVC-4 — xdg-desktop-portal backend.** Basic slice started: `lambda-portal` is a Linux session-bus service with installed D-Bus service metadata and `lambda.portal` selector metadata; it exports `org.freedesktop.impl.portal.Settings`, `org.freedesktop.impl.portal.Account`, `org.freedesktop.impl.portal.AppChooser`, `org.freedesktop.impl.portal.Inhibit`, and `org.freedesktop.impl.portal.Notification` at `/org/freedesktop/portal/desktop`. Settings supports `Read`, `ReadAll`, `SettingChanged`, and `version`, and exposes `org.freedesktop.appearance` `color-scheme`, `accent-color`, `contrast`, and `reduced-motion`. These values now prefer Shell-owned appearance config keys editable through Settings (`appearance.color_scheme`, `appearance.accent_color`, `appearance.high_contrast`, and `appearance.reduced_motion`), with the older `LAMBDA_PORTAL_*` environment variables kept as a missing-config development fallback. Account supports `GetUserInformation` with local user id, display name, and optional face-image URI; the required user consent dialog/policy is still pending. AppChooser supports the OpenURI frontend's `ChooseApplication`/`UpdateChoices` path with deterministic no-UI selection: valid `last_choice` wins, otherwise the first offered app id is returned with the activation token preserved. Inhibit records in-memory inhibit requests, exports `org.freedesktop.impl.portal.Request.Close` handles, accepts `CreateMonitor`, and tracks `QueryEndResponse`; real logind inhibitors and session state signals are still pending. Notification supports `AddNotification`, `RemoveNotification`, empty `SupportedOptions`, `version`, and `ActionInvoked`; it maps portal app/id pairs to SVC-6 daemon ids, forwards title/body/buttons through `org.freedesktop.Notifications.Notify`, reuses replacement ids, closes through `CloseNotification`, and relays daemon action signals back to portal clients. `PortalSettingsTests.cpp`, `PortalAccountTests.cpp`, `PortalAppChooserTests.cpp`, `PortalInhibitTests.cpp`, and `PortalNotificationTests.cpp` cover the backends against a private bus, and process-level private-bus smokes verify the built `lambda-portal` service name plus Settings/Account/AppChooser/Inhibit calls. Remaining finer details: validate through the real `xdg-desktop-portal` frontend with GTK/Qt apps, implement FileChooser (Open/Save -> a Lambda file-chooser surface, reusing `lambda-files` components), then Account consent UI, real logind-backed Inhibit policy/state, a real OpenURI chooser/default-app UI path, richer notification options, and frontend request/response handling. Acceptance: Firefox/Flatpak "Open File" shows Lambda's chooser; a Flatpak app launches and can pick files; dark-mode preference propagates to a GTK app.
