...
**Last updated:** 2026-06-14 (started SVC-1 D-Bus capability, SVC-2 libseat KMS device access, SVC-3 logind client basics, and SVC-4 portal Settings backend basics; integrated daily-driver gap analysis: new System Services workstream SVC-*, compositor protocol/power gates WM-11…WM-21, missing-app specs, reprioritized gate)
...
| System Services | **Started. Basic `lambda::dbus` capability exists on Linux via `sd-bus` with bus connections, sync calls, signal matches, Unix-fd passing, simple property get/set, object export, portal Settings value shapes, and event-loop fd pumping. KMS now attempts libseat-managed DRM/input opens with direct-open fallback and dispatches libseat enable/disable callbacks into the existing VT release/acquire path. A basic `lambda::system::LogindClient` can issue logind power calls, hold fd-based inhibitors, and subscribe to sleep/session lock signals against a tested D-Bus fixture. A basic `lambda-portal` service exports `org.freedesktop.impl.portal.Settings` with appearance color-scheme/accent/contrast/reduced-motion. No FileChooser/Screencast portal, notifications daemon, tray host, polkit agent, or NetworkManager/BlueZ/UPower/MPRIS/secrets/udisks clients yet. Largest remaining greenfield gap.** |
...
| P0.5 | System Services and Session | D-Bus capability; libseat/logind seat+power; portal backend (file chooser, settings, screencast); notifications daemon; tray host; idle-notify+DPMS; screen lock; polkit; network/audio/bluetooth/power/media/secrets/mounts | **Open: SVC-1 basic D-Bus capability started; SVC-2 basic libseat device path started; SVC-3 basic logind client started; SVC-4 basic portal Settings backend started; remaining services not started** |
...
This layer is mostly greenfield. A basic Linux `lambda::dbus` capability is now in tree, and the KMS backend now tries libseat-managed DRM/input opens before falling back to direct opens. There is still no portal backend, notifications daemon, tray host, polkit agent, or NetworkManager/BlueZ/UPower/MPRIS/secrets/udisks integration. The Shell's existing provider notes already name these D-Bus services as consumers — this section provides the shared capability and service implementations they depend on.
