...
| P25 | WM-COMP-26 Output layout and multi-output foundation | In progress | Added a tested single-output layout box carrying logical position, size, scale, and transform; `wl_output.geometry` and `xdg_output.logical_position/size` now consume it while staying at origin for v1 | Output/xdg-output/snapshot tests plus compositor suite | Target hardware multi-output gate before enabling multiple active outputs |
...
3. In progress: add tests for output enter/leave selection, xdg-output logical position/size calculation, and layer/window placement on a logical layout. First slice covers xdg-output logical position/size updates through the layout box.
