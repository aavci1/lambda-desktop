...
| P25 | WM-COMP-26 Output layout and multi-output foundation | In progress | Added tested selected-output layout, output-enter policy, and layer-shell logical-origin placement helpers; `wl_output.geometry`, surface enter on output bind/surface creation, `xdg_output.logical_position/size`, and layer placement now consume shared layout policy while staying single-output for v1 | Output/xdg-output/layer-shell tests plus compositor suite | Target hardware multi-output gate before enabling multiple active outputs |
...
3. In progress: add tests for output enter/leave selection, xdg-output logical position/size calculation, and layer/window placement on a logical layout. Current slices cover xdg-output logical position/size updates, single-output surface-enter policy, and layer-shell placement with non-zero logical output origins.
