...
| P23 | WM-COMP-24 Data-device and drag/drop lifecycle parity | In progress | DnD action negotiation, destination-side offer request validation, and source-side action lifecycle validation are factored into tested helpers; remaining cleanup comparison is planned | Data-device/DnD/selection tests plus compositor suite | Manual DnD/file-transfer validation if payload behavior changes |
...
7. Planned: request manual validation if file payloads, external app DnD, or visible drag icons are changed.
