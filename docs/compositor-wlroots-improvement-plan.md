...
| P24 | WM-COMP-25 Layer-shell dynamic behavior parity | In progress | Keyboard interactivity, dynamic committed layer/reserved-zone state, anchor/margin placement, and layer-surface popup reparenting are covered by tested helpers; remaining output changes, fullscreen panel reservations, focus, and output-scale updates are planned | Layer-shell tests plus compositor suite | Shell dock/topbar/fullscreen validation if placement/focus behavior changes |
...
4. Verified slice: added committed-state coverage for dynamic layer changes and dock reserved-zone anchor/margin aggregation, then moved anchor/margin placement into a tested helper used by runtime layer geometry. Remaining fullscreen panel reservations, focus, output-scale updates, and deeper placement scenarios are planned.
