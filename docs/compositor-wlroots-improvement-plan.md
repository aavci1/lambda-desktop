...
| P24 | WM-COMP-25 Layer-shell dynamic behavior parity | In progress | Keyboard interactivity, dynamic committed layer/reserved-zone state, anchor/margin placement, layer-surface popup reparenting, fullscreen dock-hide policy, and command-launcher modal focus policy are covered by tested helpers; remaining output changes and output-scale updates are planned | Layer-shell tests plus compositor suite | Shell dock/topbar/fullscreen validation if placement/focus behavior changes |
...
4. Verified slice: added committed-state coverage for dynamic layer changes and dock reserved-zone anchor/margin aggregation, moved anchor/margin placement into a tested helper used by runtime layer geometry, centralized the fullscreen dock-hide policy used by input routing and frame-callback filtering, and centralized the command-launcher modal focus policy. Remaining output-scale updates, visual fullscreen validation, and deeper placement scenarios are planned.
