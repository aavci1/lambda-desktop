...
| P24 | WM-COMP-25 Layer-shell dynamic behavior parity | In progress | Keyboard interactivity version normalization and validation are factored into tested helpers; remaining output changes, exclusive-zone recomputation, dynamic placement, and popup/menu interactions are planned | Layer-shell tests plus compositor suite | Shell dock/topbar/fullscreen validation if placement/focus behavior changes |
