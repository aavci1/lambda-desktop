...
3. In progress: moved that frame-size decision into a shared helper used by snapshots, top chrome hit testing, cutout controls hit testing, `windowGeometryFor`, and window-manager transition start-size bookkeeping. Remaining renderer/chrome/window-management consumers still need the full audit.
