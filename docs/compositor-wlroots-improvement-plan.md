...
| P26 | WM-COMP-27 Visual regression and real-app harness | In progress | Added the real-app validation matrix, a drift checker, mpv coverage, and an opt-in smoke-runner mode that starts/stops `lambda-window-manager` with CPU/KMS/pacing traces, `wayland-info` protocol registry validation, and fatal/protocol log scanning | `verify-wm-comp27-matrix.sh`, smoke-runner syntax/list/probe checks, and start-compositor guard checks pass | Target hardware visual gate remains |
