...
| P5 | WM-COMP-6 DMABUF feedback and buffer lifetime | Waiting for user validation | Automated protocol hardening is complete; target-hardware GPU-client/video validation is needed | DMABUF validation and buffer release tests pass | Validate GPU clients and video playback on target hardware |
...
6. Waiting for user validation: run GPU-client/video scenarios on target hardware.
...
| 2026-05-31 | WM-COMP-6 | Waiting for user validation | Rejected non-zero dmabuf buffer flags because the renderer does not yet implement `y_invert` or interlaced semantics; accepting them would produce incorrect output. Build passed for `lambda_tests` and `lambda-window-manager`; `./build/tests/lambda_tests --test-case="*dmabuf*"`, `./build/tests/lambda_tests --test-case="*Compositor*"`, and `./build/tests/lambda_tests --source-file-exclude="*RuntimeInputTests.cpp"` passed. Manual validation needed: run Firefox/WebGL or another GPU client, video playback, and normal window resize/use on target hardware and confirm no protocol disconnects, rendering corruption, or missing buffer updates. |
