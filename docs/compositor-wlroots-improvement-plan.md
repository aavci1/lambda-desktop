...
| P24 | WM-COMP-25 Layer-shell dynamic behavior parity | In progress | Keyboard interactivity, dynamic committed layer/reserved-zone state, and anchor/margin placement are covered by tested helpers; remaining output changes, fullscreen panel reservations, focus, output-scale updates, and popup/menu interactions are planned | Layer-shell tests plus compositor suite | Shell dock/topbar/fullscreen validation if placement/focus behavior changes |
