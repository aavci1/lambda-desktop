...
| P8 | WM-COMP-9 XDG toplevel request and configure parity | Verified slice | XDG toplevel parent state now retains only mapped parents and reparents children on null-buffer unmap; next step is configure-state parity | XDG toplevel/parent tests plus compositor suite pass | No manual gate needed for this protocol-lifecycle slice |
