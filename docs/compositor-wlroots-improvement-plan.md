...
| P5 | WM-COMP-6 DMABUF feedback and buffer lifetime | In progress | Retained dmabuf releases survive surface destroy; next review remaining GPU-client validation gaps | DMABUF validation and buffer release tests where possible | GPU-client/video validation on target hardware |
