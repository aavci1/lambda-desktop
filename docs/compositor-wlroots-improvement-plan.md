...
| P22 | WM-COMP-23 Seat focus and grab workflow parity | Verified slices | xdg-popup grabs now follow the wlroots popup-grab model for the implemented pointer/keyboard seat paths, and unmapped XDG toplevels now clear stale seat focus/order/grab/serial/activation state while deactivating pointer constraints. Broader non-popup seat-grab parity remains planned. | Seat serial, popup, window-state, pointer-constraint, config, compositor, and broader feasible tests pass | Firefox app/context menu actions and click-open submenus passed; no manual gate needed for the unmapped-toplevel cleanup slice |
...
6. Planned: define any remaining cancellation rules for destroyed focus surfaces, popup parents, layer-shell focus surfaces, and non-popup pointer/keyboard grabs outside the Firefox menu path.
7. Planned: add broader automated tests for grab start/end/cancel, destroyed-surface cleanup, popup/menu focus, and keyboard focus restoration.
8. Planned: request manual app menu/dock menu validation if runtime grab behavior changes again.
