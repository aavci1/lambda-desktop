...
| P23 | WM-COMP-24 Data-device and drag/drop lifecycle parity | In progress | DnD action negotiation, offer/source validation, single-use source lifecycle, drag-icon role validation, post-finish offer request rejection, completed-drop cleanup, post-drop offer-abort cancellation, and compositor-cancel source notification are factored into tested helpers; remaining destruction/cancel cleanup comparison is planned | Data-device/DnD/selection tests plus compositor suite | Manual DnD/file-transfer validation if payload behavior changes |
...
12. Planned: request manual validation if file payloads, external app DnD, or visible drag icons are changed.
