...
| P8 | WM-COMP-9 XDG toplevel request and configure parity | Verified slice | Min/max size-hint validation now happens on client commit, matching wlroots synced toplevel state; next step is title/app-id and parent semantics | XDG toplevel/size-hint tests plus compositor suite pass | No manual gate needed for this protocol-validation slice |
