...
- [x] Multi-dirty partial-damage verification is now enforced by `scripts/verify-frame-pacing-linux.sh`. Latest run passed on 2026-06-11 (`.debug-logs/frame-pacing-verify/20260611-164917/multi-dirty-partial`): 153 partial frames, 129 multi-rect partial frames, max logical rect count 2, 631 flips with at least two surfaces, zero fatal matches, and surface avg/max 0.175/0.431 ms under the 4.000 ms gate.
...
- [x] [Auto] Frame profile/CPU trace: `scripts/verify-frame-pacing-linux.sh` now keeps two decorated checker windows mapped, drives scripted multi-toplevel motion, and fails unless multi-rect partial frames are observed with low `surface_ms`. Latest run: 153 partial frames, 129 multi-rect partial frames, max logical rect count 2, 631 flips with at least two surfaces, and surface avg/max 0.175/0.431 ms.
