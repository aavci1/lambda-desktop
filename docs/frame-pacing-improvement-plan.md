...
- [ ] Remaining local input gap: `ydotool` and `wtype` are installed, but `evemu-event`/`evemu` and `wayland-info`/`wayland-utils` are missing. `/dev/uinput` is still `root:root` mode `0600`, and `sudo -n true` still prompts for a password, so this session cannot install the missing packages or repair `/dev/uinput` permissions. Real hardware input-driver and manual cursor visual validation still require a prepared host.
