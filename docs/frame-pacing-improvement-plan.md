...
- [x] Real evdev hardware-input validation is now repeatable through `scripts/verify-evemu-hardware-input.sh` with an explicit `LWM_EVEMU_DEVICE`. Latest run passed on 2026-06-11 (`.debug-logs/evemu-hardware-input/20260611-212551`) against `/dev/input/event6` (`GXTP7863:00 27C6:01E0 Mouse`): 60 `evemu-event` relative-motion commands, 60 hardware-cursor fast-path moves, zero fallback/unavailable moves, presentation feedback with `VSYNC|HW_CLOCK|HW_COMPLETION`, and zero fatal matches.
