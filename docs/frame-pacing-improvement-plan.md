...
- [x] Added static decorated-SHM surface cache verification to `scripts/verify-frame-pacing-linux.sh`. Latest run: 500 surface draw-cache hits, 1 miss, zero transient-chrome blocks, 524 rendered frames, CPU avg/max 7.47%/9.10%, surface avg/max 0.011/0.012 ms, present avg/max 0.209/0.217 ms.
- [x] Added server-side chrome hover/press verification to `scripts/verify-frame-pacing-linux.sh`. Latest run: synthetic close-button move, press, move-away, release, and completion events all observed; surface draw-cache reported 4 hits, 1 miss, zero transient-chrome blocks, CPU avg/max 1.30%/2.10%, and surface avg/max 0.281/0.715 ms.
