...
**Last updated:** 2026-06-17 (started SVC-1 D-Bus capability with exported-object introspection, `PropertiesChanged` helpers, async method calls with cancellable pending handles, generic variant method arguments, generic array/struct/dictionary/request-option-map value support, int16/uint16 scalar support, and shared service-loop fd pumping; SVC-2 libseat KMS device access plus safe libinput rebuild teardown on VT return; SVC-3 logind client basics with active-session discovery and Shell launcher plus visible dock power/session actions; SVC-4 portal Settings backend basics with Shell-owned appearance config, basic Account user-info replies, basic AppChooser/OpenURI selection, basic FileChooser backend request/result plumbing, basic in-memory Inhibit request handles, and basic Notification routing through SVC-6; SVC-5 basic ScreenCast request/session plumbing with deterministic development stream metadata; SVC-6 notification service basics plus common hint parsing, Shell-facing posted/action events, and a basic live Shell banner with timeout/preview policy; SVC-7 StatusNotifierWatcher basics plus Shell host registration, basic live tray glyphs, item metadata/property-refresh wiring, pixmap/tooltip property parsing, and bounded `GetAll` metadata refreshes; SVC-8 polkit agent registration/export basics; SVC-9 NetworkManager status/toggle basics with richer AP/connection/connectivity state and Shell manager/device/AP event refresh; SVC-10 BlueZ status/toggle basics with richer adapter/device/battery/disconnect state and Shell property/ObjectManager event refresh; SVC-11 UPower basics with richer Shell battery status and display-device/manager event refresh; SVC-12 MPRIS basics with Shell docklet/control integration, event refresh, selected-player capability policy, richer metadata, progress, and TrackList reads; SVC-13 in-memory Secret Service basics; SVC-14 UDisks2 basics with encrypted/job/control-result state plus Files visible-volume sidebar/actions/opt-in read-only auto-mount/event refresh; Files local Preview defaults for supported image MIME types; and shared tooltip hover propagation through nested child components; integrated daily-driver gap analysis: new System Services workstream SVC-*, compositor protocol/power gates WM-11…WM-21, missing-app specs, reprioritized gate)
...
| [conventions.md](../../docs/conventions.md) | Repository layout, CMake, namespaces, platforms, examples |
| [reactive-graph.md](../../lambda/docs/reactive-graph.md), [composites.md](../../lambda/docs/composites.md), [migrating-to-v5.md](../../lambda/docs/migrating-to-v5.md), [ui-view-body-style.md](../../lambda/docs/ui-view-body-style.md), [event_queue.md](../../lambda/docs/event_queue.md) | Lambda v5 framework reference |
...
| System Services | **Started. Basic `lambdaui::dbus` capability exists on Linux via `sd-bus` with bus connections, sync calls, signal matches, Unix-fd passing, object-path replies, object-path arrays, byte arrays/scalars/arrays, int16/uint16 scalars, managed-object dictionaries, nested `a{sv}` metadata values, generic variant method arguments, `Properties.GetAll`, `PropertiesChanged` emit/read helpers, simple property get/set, object export with basic introspection XML, portal/notification/SNI/NetworkManager/BlueZ/UPower/MPRIS/UDisks2/Secret Service value shapes, `Application` event-loop fd pumping, and shared service-loop wait/process pumping. KMS now attempts libseat-managed DRM/input opens with direct-open fallback and dispatches libseat enable/disable callbacks into the existing VT release/acquire path. A basic `lambdaui::system::LogindClient` can issue logind power/session calls, hold fd-based inhibitors, discover the current session path, and subscribe to sleep/session lock signals against a tested D-Bus fixture; Shell exposes search-driven launcher actions and a visible dock session menu for lock, log out, suspend, hibernate, restart, and power off through that client. A basic `lambda-portal` service exports `org.freedesktop.impl.portal.Settings` with appearance color-scheme/accent/contrast/reduced-motion read from Shell-owned appearance config, with environment variables retained only as a missing-config fallback, `org.freedesktop.impl.portal.Account` with local user-info replies, `org.freedesktop.impl.portal.AppChooser` with deterministic OpenURI application selection, `org.freedesktop.impl.portal.FileChooser` with OpenFile/SaveFile/SaveFiles request-result plumbing and deterministic file URI selection, `org.freedesktop.impl.portal.Inhibit` with in-memory request handles and monitor acknowledgement, `org.freedesktop.impl.portal.Notification` with add/remove/action routing through `lambda-notifications`, and `org.freedesktop.impl.portal.ScreenCast` with CreateSession/SelectSources/Start request-session plumbing plus deterministic development stream metadata. A basic `lambda-notifications` service exports `org.freedesktop.Notifications` with Notify/Close/GetCapabilities/GetServerInformation, in-memory history, action and common presentation-hint parsing, standard close/action signals, and Lambda-private posted/action routing consumed by Shell's basic live banner and by the portal Notification backend. A basic `lambda-status-notifier-watcher` service exports `org.kde.StatusNotifierWatcher` with item/host registration, properties, signals, and owner-loss pruning; Shell registers a host, watches registered item and item-property changes through the event pump, reads SNI metadata including icon pixmap arrays and tooltips through bounded `GetAll` calls, and renders live tray labels/glyphs. A basic `lambda-polkit-agent` system-bus process exports `org.freedesktop.PolicyKit1.AuthenticationAgent`, registers its object with the polkit authority for the current session/process subject, parses `BeginAuthentication` requests, and cancels cleanly until the UI/PAM path lands. A basic `lambdaui::system::NetworkManagerClient` reads manager/device/Wi-Fi/AP status, connectivity, metered state, visible APs, saved and active connections, and VPN state, formats Shell network/Wi-Fi labels, can toggle Wi-Fi enabled state, and can watch manager/device/AP/active-connection/settings changes; Shell network/Wi-Fi status now prefers NetworkManager with sysfs fallback, the network docklet can toggle Wi-Fi, and production Shell refreshes on those NetworkManager events through the D-Bus event pump. A basic `lambdaui::system::BlueZClient` reads adapter/device power, discovery, pairing/trust/block, icon/class/appearance, service, signal, battery, and disconnect-reason state, formats Shell Bluetooth labels, can toggle adapter power, and can watch adapter/device/battery property, disconnect, and ObjectManager add/remove changes; Shell Bluetooth status now prefers BlueZ with rfkill/sysfs fallback, the Bluetooth docklet can toggle adapters, and production Shell refreshes on those BlueZ events through the D-Bus event pump. A basic `lambdaui::system::UPowerClient` reads display-device battery state, warning level, and manager AC state, can watch display-device, manager, and device inventory changes, Shell battery status now prefers UPower with sysfs fallback, preserves structured charge/source/warning/time/icon state for Shell models, and production Shell refreshes on those UPower events through the D-Bus event pump. A basic `lambdaui::system::MPRISClient` discovers session media players, reads richer now-playing metadata/capabilities, art cache keys, desktop icon names, progress, optional TrackList metadata, prunes stale player snapshots for selection/status, applies playing/paused/controllable precedence, gates controls by per-player capability, can send transport/volume controls, can watch player/TrackList property and signal changes, and Shell now exposes a media docklet with play/pause/skip actions that refreshes from the D-Bus event pump. A basic `lambda-secrets` service exposes an in-memory `org.freedesktop.secrets` default collection, plain sessions, item create/search/get/replace/delete, default alias, and item properties for libsecret-style development flows. A basic `lambdaui::system::UDisks2Client` reads drives, visible filesystem volumes, encrypted state, mount options, and job progress, can mount/unmount/eject/unlock/lock/cancel with classified operation failures, can watch ObjectManager/block/filesystem/drive/encrypted/job changes, and Files now shows visible volumes in its sidebar with mount/open/unmount/force-unmount/eject/cancel actions, status subtitles, classified error reporting, opt-in read-only auto-mount policy, and event-driven refresh. No interactive FileChooser, real PipeWire-backed ScreenCast/RemoteDesktop portal, polkit UI/PAM path, or encrypted persistent keyring yet. Largest remaining greenfield gap.** |
| Shell | App registry, dock, IPC, config, icons, status fallback, a basic live notification banner with timeout/preview/action policy, and basic live tray metadata labels/glyphs exist; full notification center, real tray icons/menus, clipboard, and most provider control remain, gated on System Services. |
...
| Files | Live file browser/manager exists; model layer is ahead of live UI for several commands; basic removable-media sidebar rows, mount/open/unmount/eject actions, and opt-in read-only auto-mount policy now use udisks2 (SVC-14), with unlock UI and real hardware validation still open. |
...
| P0.5 | System Services and Session | D-Bus capability; libseat/logind seat+power; portal backend (file chooser, settings, screencast); notifications daemon; tray host; idle-notify+DPMS; screen lock; polkit; network/audio/bluetooth/power/media/secrets/mounts | **Open: SVC-1 basic D-Bus capability started; SVC-2 basic libseat device path started; SVC-3 basic logind client and Shell launcher power actions started; SVC-4 basic portal Settings/Account/AppChooser/FileChooser/Inhibit/Notification backend started; SVC-5 basic ScreenCast backend plumbing started; SVC-6 basic notification service started; SVC-7 basic watcher/metadata host started; SVC-8 basic polkit agent registration started; SVC-9 basic NetworkManager status client started; SVC-10 basic BlueZ status client started; SVC-11 basic UPower client started; SVC-12 basic MPRIS client started; SVC-13 basic in-memory Secret Service started; SVC-14 basic UDisks2 client started; several portal/session details remain not started** |
...
- **Phase B — make the session safe and modern:** SVC-3 (logind power/inhibit) + WM-13 (idle-notify) + WM-14 (DPMS); WM-11 (session-lock) + lock app + PAM; WM-12 (screen capture) + SVC-5 (screencast portal) + SVC-4 (portal file chooser/settings); SVC-6 (notifications daemon). Basic SVC-3 client plumbing with current-session discovery plus Shell launcher and visible dock power/session actions, SVC-4 Settings/Account/AppChooser/FileChooser/Inhibit/Notification backend plumbing with Shell-owned appearance config and SVC-6 routing, SVC-5 ScreenCast request/session plumbing, and SVC-6 notification service plus basic Shell live-banner plumbing are now in tree; Shell power confirmations/error states, Shell/lock/idle integration, real logind validation, full Shell notification center/actions, interactive FileChooser UI, and real ScreenCast capture/PipeWire work remain.
- **Phase C — connectivity and polish:** SVC-9 (NetworkManager) + Shell Wi-Fi; SVC-7 (tray host); SVC-8 (polkit); SVC-10/11/12/13/14 (BlueZ/UPower/MPRIS/secrets/udisks); SVC-15 (theming bridge); WM-15/16/17/18 (foreign-toplevel/output-management/data-control/gamma). The basic SVC-7 watcher/registry daemon plus Shell host registration, item metadata/pixmap/tooltip reads, property refreshes, and tray labels/glyphs, SVC-8 polkit registration/export daemon, SVC-9 status client with richer AP/connection/connectivity state and Shell manager/device/AP event refresh, SVC-10 status client with richer adapter/device/battery/disconnect state and Shell property/ObjectManager event refresh, SVC-11 display-device battery client with richer Shell battery status and Shell display-device/manager event refresh, SVC-12 media client and Shell docklet/control path with event refresh, selected-player capability policy, richer metadata, and TrackList reads, SVC-13 in-memory Secret Service daemon, and SVC-14 removable-volume client with encrypted/job/control-result state plus Files visible-volume sidebar/actions/opt-in read-only auto-mount/event refresh are now in tree; real tray image/theme/menu/action hosting, AP/connect flows, Bluetooth pairing/connect flows, polkit UI/PAM, persistent encrypted secrets, richer Shell now-playing/media keys, Files unlock-passphrase UI, and real removable-media validation remain.
...
- SVC-4: xdg-desktop-portal backend — FileChooser, Settings (color-scheme/accent), OpenURI/AppChooser, Inhibit, Account, Notification routing (basic started).
...
- SVC-9: NetworkManager client (Wi-Fi/ethernet/VPN) for Shell quick settings + Settings network page; basic status/toggle and richer AP/connection state slices started.
- SVC-10: BlueZ client (adapters, discovery, pairing agent, connect); basic status/toggle and richer adapter/device state slices started.
...
- SVC-14: udisks2 client — removable volumes, mount/unmount/eject, Files integration; basic client/control, Files sidebar/action, and opt-in read-only auto-mount slice started.
...
This layer is mostly greenfield. A basic Linux `lambdaui::dbus` capability is now in tree, the KMS backend now tries libseat-managed DRM/input opens before falling back to direct opens, `lambda-portal` covers the portal Settings backend, basic Account user-info replies, basic AppChooser/OpenURI selection, basic FileChooser request/result plumbing, basic ScreenCast request/session plumbing, basic in-memory Inhibit request handling, and basic Notification routing through `lambda-notifications`, `lambda-notifications` covers the first Freedesktop notification service slice, `lambda-status-notifier-watcher` plus Shell host registration, item metadata/pixmap/tooltip reads, property refreshes, and tray labels/glyphs covers the first tray watcher/host slice, `lambda-polkit-agent` covers the first polkit registration/export slice, `lambdaui::system::NetworkManagerClient` covers the first network-status client slice, `lambdaui::system::BlueZClient` covers the first Bluetooth-status client slice, `lambdaui::system::UPowerClient` covers the first power-status client slice, `lambdaui::system::MPRISClient` plus Shell docklet wiring covers the first media-status/control slice, `lambda-secrets` covers the first in-memory Secret Service slice, and `lambdaui::system::UDisks2Client` plus Files sidebar action wiring covers the first removable-volume client slice. There is still no interactive FileChooser UI, real PipeWire-backed ScreenCast/RemoteDesktop portal, polkit UI/PAM authentication path, or encrypted persistent keyring. The Shell's existing provider notes already name these D-Bus services as consumers — this section provides the shared capability and service implementations they depend on.

Process/library shape:

- Add a framework-level `lambdaui::dbus` capability (recommended backend: `sd-bus` from systemd/elogind — present on the target distro, ergonomic, integrates with an external event loop via `sd_bus_get_fd`/`sd_bus_process`/`sd_bus_get_events`; alternative: `sdbus-c++`). It must support: connecting to the session and system buses, async + sync method calls, signal subscription, property get/set/watch, and exporting objects/interfaces (needed for being a service: notifications, StatusNotifierWatcher, portal backend, polkit agent).
- First-party service processes where a daemon boundary is cleaner than in-process: a `lambda-portal` backend binary, `lambda-polkit-agent`, `lambda-secrets`, and the lock app `lambda-lock`. Bus clients (logind, NetworkManager, BlueZ, UPower, MPRIS, and udisks) can be in-process libraries consumed by Shell/Settings/Files.

Workstreams:

- **SVC-1 — D-Bus capability.** Basic slice started: `lambdaui::dbus` opens session/system/custom-address buses through `sd-bus`, exposes sync and async method calls with basic-value arguments including Unix fds, explicit variant arguments, cancellable pending-call handles, async success/error reply delivery, object paths, object-path arrays, string arrays, byte arrays/scalars/arrays, int16/uint16 scalars, RGB tuples, generic arrays, generic structs, generic dictionaries, nested variants, portal-style request option maps, empty `a{sv}` hints dictionaries, variant replies, `a{sv}` property dictionaries, namespaced portal Settings dictionaries, managed-object dictionaries for ObjectManager, nested `a{sv}` values for metadata, signal matching, simple property get/set/`GetAll`, `PropertiesChanged` emit/read helpers, object export with method/property handlers and basic `Introspectable` XML, signal emission, unique-name lookup, fd/events/process/flush helpers, `BusEventPump` integration with `Application::registerEventPollSource`, and a shared service-loop `waitAndProcess` helper used by first-party D-Bus daemons and private-bus fixtures. Focused integration tests cover `Peer.Ping`, exported sync and async method calls, async method errors and cancellation, exported-object introspection, Unix-fd round trips, object-path replies, exported property get/set/GetAll, generic portal option-map round trips with nested arrays/structs/dictionaries, variant method arguments, `PropertiesChanged` helpers, signal delivery, logind `GetSessionByPID`, portal Settings D-Bus shapes, notification D-Bus shapes, StatusNotifierWatcher D-Bus shapes, polkit subject/authentication-agent D-Bus shapes, Secret Service plain-session/secret-map shapes, NetworkManager device/AP D-Bus shapes, BlueZ ObjectManager shapes including int16/uint16 adapter/device metadata, MPRIS metadata shapes, UDisks2 block/filesystem shapes, and UPower display-device D-Bus shapes against a private real bus when `dbus-daemon` can run. Remaining finer details: detailed method argument introspection metadata, additional service-client fixtures, compositor-runtime fd pumping, and real-bus validation.
- **SVC-2 — Seat/session via libseat.** Basic slice started: `lambda-window-manager` links `libseat`, opens a seat at KMS startup when available, opens DRM and libinput devices through `libseat_open_device` with direct-open fallback, tracks seat-managed device IDs for close, routes the libseat fd through the KMS poll loop, maps `enable_seat`/`disable_seat` callbacks onto the existing DRM-master/libinput suspend/resume VT paths, rebuilds libinput on seat enable so keyboard/mouse fds are reacquired through libseat, explicitly removes path-added libinput devices and drains pending device events during input teardown so failed reopen attempts do not crash, and routes Ctrl-Alt-F<n> plus Alt-Left/Right through `libseat_switch_session` with a kernel `VT_ACTIVATE` fallback when libseat is unavailable. Remaining finer details: close/reopen the libseat-managed DRM device by recreating the atomic presenter/GBM resources on enable, remove/limit direct-open fallback once hardware validation passes, and validate unprivileged launch + VT switch on target hardware. Acceptance: `lambda-window-manager` launches as an unprivileged user inside a logind session with no manual device permissions; Ctrl-Alt-F<n> switches VTs and returns cleanly; the GPU is released/reacquired without corruption.
- **SVC-3 — logind client.** Basic slice started: `lambdaui::system::LogindClient` connects through `lambdaui::dbus`, calls `Suspend`/`Hibernate`/`PowerOff`/`Reboot`, can ask the current session to `Lock` or `Terminate`, takes fd-based `Inhibit` locks, resolves session object paths through `GetSessionByPID`, subscribes to manager `PrepareForSleep`, and watches explicit or current-session `Lock`/`Unlock` signals. Shell command-launcher search and a visible dock session menu now expose lock, log out, suspend, hibernate, restart, and power-off actions through the client. `LogindTests.cpp` covers the power/session calls, inhibitor fd plumbing, current-session discovery, sleep signal, and session lock/unlock signals against a fake `org.freedesktop.login1` on a private bus, and Shell tests cover launcher/session-menu action discovery and mounting. Remaining finer details: add confirmations and error states, wire the client into WM idle/DPMS behavior and the lock app, hold the real delay inhibitors for `handle-power-key`, `handle-lid-switch`, and `handle-suspend-key`, honor `IdleAction` together with WM-13, and validate against a real system bus. Acceptance: closing the lid suspends (and locks first); the power key triggers the Shell power menu; `loginctl lock-session` locks; resume-from-suspend shows the lock screen.
- **SVC-4 — xdg-desktop-portal backend.** Basic slice started: `lambda-portal` is a Linux session-bus service with installed D-Bus service metadata and `lambda.portal` selector metadata; it exports `org.freedesktop.impl.portal.Settings`, `org.freedesktop.impl.portal.Account`, `org.freedesktop.impl.portal.AppChooser`, `org.freedesktop.impl.portal.FileChooser`, `org.freedesktop.impl.portal.Inhibit`, `org.freedesktop.impl.portal.Notification`, and `org.freedesktop.impl.portal.ScreenCast` at `/org/freedesktop/portal/desktop`. Settings supports `Read`, `ReadAll`, `SettingChanged`, and `version`, and exposes `org.freedesktop.appearance` `color-scheme`, `accent-color`, `contrast`, and `reduced-motion`. These values now prefer Shell-owned appearance config keys editable through Settings (`appearance.color_scheme`, `appearance.accent_color`, `appearance.high_contrast`, and `appearance.reduced_motion`), with the older `LAMBDA_PORTAL_*` environment variables kept as a missing-config development fallback. Account supports `GetUserInformation` with local user id, display name, and optional face-image URI; the required user consent dialog/policy is still pending. AppChooser supports the OpenURI frontend's `ChooseApplication`/`UpdateChoices` path with deterministic no-UI selection: valid `last_choice` wins, otherwise the first offered app id is returned with the activation token preserved. FileChooser supports the backend `OpenFile`, `SaveFile`, and `SaveFiles` methods, exports closeable request handles, normalizes deterministic selections to `file://` URIs, passes through choice/current-filter result metadata, supports request-option and development-environment fallbacks, and returns no-selection responses when no deterministic selection exists; the interactive Lambda chooser UI is still pending. Inhibit records in-memory inhibit requests, exports `org.freedesktop.impl.portal.Request.Close` handles, accepts `CreateMonitor`, and tracks `QueryEndResponse`; real logind inhibitors and session state signals are still pending. Notification supports `AddNotification`, `RemoveNotification`, empty `SupportedOptions`, `version`, and `ActionInvoked`; it maps portal app/id pairs to SVC-6 daemon ids, forwards title/body/buttons through `org.freedesktop.Notifications.Notify`, reuses replacement ids, closes through `CloseNotification`, and relays daemon action signals back to portal clients. ScreenCast supports the backend `CreateSession`, `SelectSources`, and `Start` methods, exports closeable request/session handles, advertises monitor-only hidden-cursor capability properties, and returns deterministic development stream metadata only when explicitly configured; real capture and PipeWire publishing are tracked in SVC-5. `PortalSettingsTests.cpp`, `PortalAccountTests.cpp`, `PortalAppChooserTests.cpp`, `PortalFileChooserTests.cpp`, `PortalInhibitTests.cpp`, `PortalNotificationTests.cpp`, and `PortalScreenCastTests.cpp` cover the backends against a private bus, and process-level private-bus smokes verify the built `lambda-portal` service name plus Settings/Account/AppChooser/FileChooser/ScreenCast/Inhibit calls. Remaining finer details: validate through the real `xdg-desktop-portal` frontend with GTK/Qt apps, replace the deterministic FileChooser fallback with a Lambda file-chooser surface reusing `lambda-files` components, then Account consent UI, real logind-backed Inhibit policy/state, a real OpenURI chooser/default-app UI path, richer notification options, and frontend request/response handling. Acceptance: Firefox/Flatpak "Open File" shows Lambda's chooser; a Flatpak app launches and can pick files; dark-mode preference propagates to a GTK app.
- **SVC-5 — Screencast/RemoteDesktop portal + PipeWire.** Basic slice started: `lambda-portal` exports `org.freedesktop.impl.portal.ScreenCast` with `CreateSession`, `SelectSources`, `Start`, read-only capability properties, closeable request/session handles, deterministic development stream metadata from `LAMBDA_PORTAL_SCREENCAST_*`, private-bus coverage, and a process smoke. Remaining finer details: replace the deterministic development stream with WM-12 dmabuf capture, publish frames through a `libpipewire` stream so browsers and conferencing apps can share the screen, provide a source-picker UI (whole output / window / region), and add the `RemoteDesktop` backend once input-injection policy exists. Acceptance: Firefox/Chromium and OBS (PipeWire source) can share a screen/window via the portal.
- **SVC-6 — Notifications daemon.** Basic slice started: `lambda-notifications` is a Linux session-bus service with installed D-Bus activation metadata; it exports `org.freedesktop.Notifications` at `/org/freedesktop/Notifications`, implements `Notify`, `CloseNotification`, `GetCapabilities`, and `GetServerInformation`, stores an in-memory history with replacement support, parses actions plus common presentation hints (urgency, category, desktop-entry, image/icon path/data, transient/resident/action-icon flags, sound metadata/suppression, and x/y placement), emits `NotificationClosed` and `ActionInvoked`, and emits a Lambda-private `NotificationPosted` signal for Shell observers. Shell now consumes that signal through a session-bus event pump, upserts notifications by daemon id, applies enabled/DND/history-limit policy, shows a basic top-right live banner, honors banner timeout and preview-visibility config, renders up to two action buttons, routes action clicks through a private daemon `InvokeAction` method so the daemon emits `ActionInvoked`, routes banner dismissals to `CloseNotification`, and hides visible banner state when `NotificationClosed` arrives. The SVC-4 portal Notification backend now routes `AddNotification`, replacement, close, and action relay through this daemon. `NotificationsTests.cpp` covers the spec methods/signals, Shell-facing posted signal, hint metadata parsing/replacement reset, and private action invoke path against a private bus; Shell model tests cover daemon-id upsert, action replacement, and banner-timeout policy; `PortalNotificationTests.cpp` covers portal routing; and a process-level private-bus smoke verifies the built daemon accepts a real `Notify` call through `gdbus`. Remaining finer details: full notification-center UI, richer action layout, service-side timeout expiry, persistence/history grouping, and Shell presentation of parsed icon/image metadata. Acceptance: `notify-send "x"` shows a banner; actions invoke; DND suppresses; history shows past notifications.
- **SVC-7 — StatusNotifierWatcher + tray host.** Basic slice started: `lambda-status-notifier-watcher` is a Linux session-bus service with installed D-Bus activation metadata; it exports `org.kde.StatusNotifierWatcher` at `/StatusNotifierWatcher`, implements `RegisterStatusNotifierItem`, `RegisterStatusNotifierHost`, `RegisteredStatusNotifierItems`, `IsStatusNotifierHostRegistered`, `ProtocolVersion`, `StatusNotifierItemRegistered`, `StatusNotifierItemUnregistered`, and `StatusNotifierHostRegistered`, supports path-only item registrations by using the sender unique name, and prunes registered services on `NameOwnerChanged`. Shell now has a small `StatusNotifierWatcherClient`, registers a host, reads registered item addresses and `org.kde.StatusNotifierItem`/`org.freedesktop.StatusNotifierItem` metadata through bounded `Properties.GetAll` calls, watches item register/unregister and item `PropertiesChanged` events through `BusEventPump`, and renders live tray labels/glyphs in the dock status grid from category, title/id, status, icon-name, tooltip, attention-icon, pixmap availability, menu-path, and `ItemIsMenu` metadata. `StatusNotifierWatcherTests.cpp` covers properties, `GetAll`, host/item registration signals, path-only registration, owner-loss removal, client read/watch paths, SNI property reads, icon/overlay/attention pixmap arrays, tooltip struct parsing, missing-item unavailable metadata, and item property-change refresh signals against a private bus; Shell dock tests cover the expanded status grid; and a process-level private-bus smoke verifies the built daemon answers a real `ProtocolVersion` property query through `gdbus`. Remaining finer details: resolving parsed icon names/pixmap bytes into Shell-rendered themed images, overlay/attention image presentation, `com.canonical.dbusmenu` menus, item activation/context/scroll actions, async/debounced refresh behavior, and real tray-app validation. Acceptance: Steam/Discord/Telegram/nm-applet-style tray icons appear and their menus work.
- **SVC-8 — polkit agent.** Basic slice started: `lambda-polkit-agent` is a Linux system-bus process target that exports `org.freedesktop.PolicyKit1.AuthenticationAgent` at `/org/lambda/PolicyKit1/AuthenticationAgent`, registers/unregisters that object with `org.freedesktop.PolicyKit1.Authority`, builds documented `unix-session` subjects from `XDG_SESSION_ID` with a `unix-process` PID/UID/start-time fallback, parses `BeginAuthentication` action/message/icon/details/cookie/identity inputs, records cancellation requests, and returns `org.freedesktop.PolicyKit1.Error.Cancelled` until the UI/PAM path exists instead of hanging privileged callers. `PolkitAgentTests.cpp` covers subject serialization, private-bus authority registration/unregistration, `BeginAuthentication` parsing, and `CancelAuthentication`. Remaining finer details: launch the agent with the Lambda session, present a Lambda authentication dialog, run `polkit-agent-helper-1`/PAM for selected identities, report success through `AuthenticationAgentResponse2`/`AuthenticationAgentResponse3`, handle duplicate/fallback agents, and validate against the real system bus. Acceptance: a `pkexec`/privileged action prompts for a password and succeeds/fails correctly.
- **SVC-9 — NetworkManager client.** Basic slice started: `lambdaui::system::NetworkManagerClient` connects through `lambdaui::dbus`, calls `GetDevices`, reads manager `State`, `Connectivity`, `Metered`, `NetworkingEnabled`, `WirelessEnabled`, `WirelessHardwareEnabled`, and `ActiveConnections`, reads device `Interface`, `DeviceType`, and `State`, reads wireless `ActiveAccessPoint` plus visible APs, reads AP identity/security/signal metadata, reads active-connection state/default/VPN/device details, reads saved connection settings through `org.freedesktop.NetworkManager.Settings`, formats connected/connecting/off/unavailable Shell labels, can set `WirelessEnabled`, and can watch manager/device/AP/active-connection/settings changes plus device/AP/connection add-remove signals. Shell network/Wi-Fi status now prefers NetworkManager on the real `/sys` path, falls back to the existing sysfs scan when NetworkManager is unavailable, routes the network docklet primary action to Wi-Fi enabled toggling when hardware is present, and production Shell subscribes to those NetworkManager events through `BusEventPump` so global network/Wi-Fi enablement, active interface, SSID, and signal changes can refresh without waiting for the polling timer. `NetworkManagerTests.cpp` covers manager/device/AP reads, visible AP enumeration, saved and active connections, connectivity/metered/VPN state, SSID/signal formatting, connecting/off mapping, Wi-Fi property writes, manager/device/AP/active-connection/settings change signals, and device/AP/connection add-remove signals against a fake `org.freedesktop.NetworkManager` on a private bus. Remaining finer details: access-point connect/disconnect with a secrets path, Settings network page, and real system-bus validation. Acceptance: connect to a WPA2 network from the desktop; state updates live in the docklet.
- **SVC-10 — BlueZ client.** Basic slice started: `lambdaui::system::BlueZClient` connects through `lambdaui::dbus`, calls ObjectManager `GetManagedObjects`, reads adapter `Address`, `AddressType`, `Name`, `Alias`, `Modalias`, `PowerState`, `Class`, `Powered`, `Discoverable`, `Pairable`, `Connectable`, `Discovering`, `UUIDs`, and `Roles`, reads device `Address`, `AddressType`, `Alias`, `Name`, `Icon`, `Class`, `Appearance`, `UUIDs`, `Adapter`, `Paired`, `Connected`, `Trusted`, `Blocked`, `LegacyPairing`, `Modalias`, `ServicesResolved`, `WakeAllowed`, `RSSI`, `TxPower`, and Battery1 `Percentage`/`Source`, formats unavailable/off/on/connected-device Shell labels, can set adapter `Powered`, and can watch adapter/device/battery property changes, Device1 `Disconnected` reason signals, and ObjectManager add/remove signals. Shell Bluetooth status now prefers BlueZ on the real `/sys` path, falls back to the existing rfkill/sysfs scan when BlueZ is unavailable, routes the Bluetooth docklet primary action to toggling known adapters powered on/off, and production Shell subscribes to those BlueZ events through `BusEventPump` so power/connection and adapter/device arrival/removal changes can refresh without waiting for the polling timer. `BlueZTests.cpp` covers richer adapter/device/battery reads, status formatting, adapter power writes, unsupported ObjectManager property skipping, adapter/device/battery change signals, disconnect reason signals, and ObjectManager add/remove signals against a fake `org.bluez` on a private bus. Remaining finer details: discovery controls, pairing agent, pair/unpair, trust/block controls, connect/disconnect, Settings Bluetooth page, and real system-bus validation. Acceptance: pair and connect a Bluetooth device.
- **SVC-11 — UPower client.** Basic slice started: `lambdaui::system::UPowerClient` connects through `lambdaui::dbus`, calls `GetDisplayDevice`, reads display-device `IsPresent`, `Percentage`, `State`, `WarningLevel`, `TimeToEmpty`, `TimeToFull`, and `IconName`, reads manager `OnBattery`, and can watch display-device `PropertiesChanged`, manager `PropertiesChanged`, `DeviceAdded`, and `DeviceRemoved`; Shell battery status now prefers UPower on the real `/sys` path, falls back to the existing sysfs scan when UPower is unavailable, preserves structured charge state, power source, warning level, time-to-empty/full, and icon-name hints in Shell models, and production Shell subscribes to those UPower events through `BusEventPump` so battery and AC/device changes can refresh without waiting for the polling timer. `UPowerTests.cpp` covers display-device reads including warning level, status formatting, display-device/manager property changes, and device add/remove signals against a fake `org.freedesktop.UPower` on a private bus; `ShellSystemStatusTests.cpp` covers sysfs fallback plus UPower-to-Shell battery status mapping; `ShellModelsTests.cpp` covers structured battery status propagation into Shell status modules and docklet icon selection. Remaining finer details: AC/lid beyond display-device summaries, multiple-device summaries, power-mode policy, real system-bus validation, and richer quick-settings UI. Acceptance: the battery docklet updates on AC plug/unplug and percentage changes without polling.
- **SVC-12 — MPRIS + media keys.** Basic slice started: `lambdaui::system::MPRISClient` connects through `lambdaui::dbus`, discovers `org.mpris.MediaPlayer2.*` names through the session bus, reads root `Identity`/`DesktopEntry`, reads player `PlaybackStatus`, `Metadata`, `Volume`, `Position`, rate/shuffle, and capability properties, parses richer metadata including album artists, genres, art URL cache keys, source URL, content-created timestamp, disc/track numbers, and track length, computes progress from position/length, derives desktop icon names, formats a now-playing label, reads optional TrackList track ids/metadata/editability, selects among multiple players with stale-snapshot pruning plus playing/paused/controllable precedence, gates controls by each selected player's capabilities, can send `PlayPause`, `Play`, `Pause`, `Stop`, `Next`, `Previous`, and `Volume` writes, and can watch player/TrackList `PropertiesChanged`, `Seeked`, TrackList mutation signals, and MPRIS `NameOwnerChanged` events. Shell system status reads live MPRIS on the live system path, renders a media docklet, routes docklet primary/secondary/scroll actions to supported play/pause, next, and previous commands on the selected active player, and production Shell subscribes to player changes through `BusEventPump` so media changes can refresh without waiting for the polling timer. `MPRISTests.cpp` covers player discovery, richer metadata parsing, progress/cache/icon helpers, TrackList reads/signals, active-player selection, stale-player pruning, capability gating, status formatting, transport calls, volume writes, and property/seek/name change signals against a fake MPRIS player on a private bus. Remaining finer details: full Shell now-playing UI, XF86 media-key routing, real artwork file caching/display, TrackList UI/action support, and real player validation. Acceptance: media keys control the active player; the bar shows the current track and can pause/skip.
- **SVC-13 — Secret Service / keyring.** Basic slice started: `lambda-secrets` is a Linux session-bus daemon with D-Bus activation metadata for `org.freedesktop.secrets`. It exports `org.freedesktop.Secret.Service` at `/org/freedesktop/secrets`, supports the required plain `OpenSession` variant handshake, exposes the default collection and `/org/freedesktop/secrets/aliases/default`, implements in-memory item create/search/get/replace/delete flows, returns no-prompt paths for basic operations, and exports item label/attributes/timestamp properties. `SecretsTests.cpp` covers the libsecret-style development flow against a private real bus: plain session open, default alias read, item create through the alias path, service search, `GetSecrets`, item property reads, replace, item `GetSecret`, delete, and post-delete search. Remaining finer details: encrypted persistent storage, login-tied unlock, real lock/unlock state, prompt objects, session cleanup on disconnect, stricter errors, libsecret/real-app validation, and a production decision on native daemon versus running/bundling an external provider such as `gnome-keyring-daemon`. Acceptance: an app storing/reading a secret via libsecret succeeds; the store unlocks at login.
- **SVC-14 — udisks2 client + Files integration.** Basic slice started: `lambdaui::system::UDisks2Client` connects through `lambdaui::dbus`, calls ObjectManager `GetManagedObjects`, reads drives, visible filesystem block devices, encrypted volumes, cleartext mappings, userspace mount options, and job progress/cancelability, decodes UDisks byte-array paths and `MountPoints`, filters hidden/system volumes, formats volume names, can call `Filesystem.Mount`, `Filesystem.Unmount`, `Drive.Eject`, `Encrypted.Unlock`, `Encrypted.Lock`, and `Job.Cancel`, exposes non-throwing operation results for busy-device, authorization, and cancellation errors, and can watch ObjectManager/property changes across block, filesystem, drive, encrypted, and job interfaces. Files now surfaces mounted, unmounted, locked, and cleartext visible volumes in the sidebar, shows status subtitles for mounted/locked/job state, routes open/mount/unmount/force-unmount/eject/cancel actions through UDisks2 result handling, reports storage errors through the existing Files error surface, applies a disabled-by-default `auto_mount_removable` policy that only plans user-visible idle hint-auto filesystem mounts and uses read-only `ro` mount options by default through `auto_mount_read_only`, and refreshes those places from UDisks2 events through `BusEventPump`. `UDisks2Tests.cpp` covers visible-volume filtering, drive/volume/encryption/job metadata, mount point decoding, mount options, forced busy-device unmount retry state, authorization failure state, unlock/lock calls, cancel calls, eject calls, and ObjectManager/property change signals against a fake `org.freedesktop.UDisks2` on a private bus; `FilesStoreTests.cpp` covers visible-volume sidebar state, context commands, action/result mapping, preference persistence, conservative auto-mount planning, and mounted-volume sidebar placement. Remaining finer details: Files unlock/passphrase UI and real removable-media validation. Acceptance: insert a USB stick → it appears in Files → open → unmount/eject safely.
- **SVC-15 — Toolkit theming bridge.** Basic portal Settings slice started through SVC-4: external clients can read Shell-owned color-scheme, accent, high-contrast, and reduced-motion values from the Lambda portal backend once staged with `xdg-desktop-portal`. Remaining finer details: validate the frontend portal with real GTK/Qt apps, add cursor/font exposure, and provide an XSettings/gsettings fallback where needed, so toolkit apps match the desktop and follow dark-mode. Acceptance: switching the desktop to dark mode flips a GTK app and a Qt app to dark; cursor theme/size and default font propagate.
...
- Network/Wi-Fi: SVC-9 (NetworkManager). Basic Shell status prefers NetworkManager with `/sys/class/net` fallback, the network docklet can toggle Wi-Fi enabled, and the client enumerates visible APs, saved/active connections, connectivity, metered, and VPN state; access-point connect/disconnect and full management remain for Settings/quick settings.
- Bluetooth: SVC-10 (BlueZ). Basic Shell status prefers BlueZ with rfkill/sysfs fallback, the Bluetooth docklet can toggle adapter power, and the client enumerates richer adapter/device/battery/disconnect state; pairing and device management remain for Settings/quick settings.
- Media: SVC-12 (MPRIS). Basic client/status/control plumbing exists, the dock can show now-playing with capability-gated play/pause/skip actions, player changes refresh through the D-Bus event pump, and the client now preserves richer metadata/progress/TrackList state; richer now-playing UI and media-key routing remain.
...
- **New (gap analysis):** removable-media support via SVC-14 — visible mounted/removable volume places, mount-on-open, unmount/force-unmount, eject, cancel, error mapping, and disabled-by-default read-only auto-mount policy are in tree; encrypted unlock UI and real hardware validation remain.

Validation: as previously specified (model, operations, trash, selection, preferences, open-with fixtures including local Preview image defaults, MIME parsing, URI-list clipboard, icon fallback, grid layout). Add: removable-volume enumerate/mount/unmount/force-unmount/eject/cancel/error mapping and auto-mount policy against SVC-14 fixtures; manual USB insert/open/eject.
