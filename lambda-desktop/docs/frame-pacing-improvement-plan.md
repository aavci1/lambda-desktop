...
**Scope:** `lambda-desktop/apps/lambda-window-manager/Compositor/`, `lambda/src/Platform/Linux/` (KMS + Wayland), `lambda/src/Graphics/Vulkan/`, `lambda/src/Graphics/Metal/`, `lambda/src/Platform/Mac/`, `lambda/src/SceneGraph/`, and `lambda/src/UI/Application.*`.
...
- [x] Added `lambda-desktop/scripts/verify-wayland-client-pacing.sh` for FP-14 Linux client pacing under headless Weston GL. Latest run passed on 2026-06-11 (`.debug-logs/wayland-client-pacing/20260611-163744`): 47 `frame-done` callbacks, 46 matched `app-render: present` pairs, 47 `vulkan-present-detail` samples, 46 immediate flushes, zero fatal matches, frameDone-to-present avg/max 1.340/5.637 ms under the 8 ms gate.
...
- [x] FP-3 Wayland resize stderr quietness is now enforced by `lambda-desktop/scripts/verify-wayland-resize-quiet.sh`. Latest run passed on 2026-06-11 (`.debug-logs/wayland-resize-quiet/20260611-190522`) under headless Weston GL: `lambda-terminal` completed 90 scripted self-resize steps with `LAMBDA_RESIZE_TRACE=0`, zero fatal matches, and zero `resize-trace`/`vulkan-recreate-swapchain`/swapchain-extent stderr noise.
- [x] FP-7 glass-terminal partial-damage measurement is now enforced by `lambda-desktop/scripts/verify-frame-pacing-linux.sh`. Latest run passed on 2026-06-11 (`.debug-logs/frame-pacing-verify/20260611-200703/glass-terminal-partial`): 651 partial frames, zero forced-full partial frames, one static decorated glass client presented in both runs, terminal perf traces observed in both runs, warm partial `background_ms`/`surface_ms`/`total_ms` averages 0.008/0.161/0.651 ms versus forced-full 0.009/0.177/0.676 ms.
- [x] FP-13 Wayland resize recreate verification is now enforced by `lambda-desktop/scripts/verify-wayland-resize-recreate.sh`. Latest run passed on 2026-06-11 (`.debug-logs/wayland-resize-recreate/20260611-201230`) under headless Weston GL: 90 scripted resize steps, 2 swapchain recreates, 3 dirty resize decisions, 88 clean no-recreate resize decisions, recreate elapsed max 0.831 ms, and retire-queue max 0.001 ms.
- [x] FP-14 FIFO-only Wayland client pacing is now enforced by `lambda-desktop/scripts/verify-wayland-client-pacing.sh`. Latest run passed on 2026-06-11 (`.debug-logs/wayland-client-pacing/20260611-201500`) under headless Weston GL with `LAMBDA_VULKAN_FORCE_FIFO_PRESENT_MODE=1`: 21 frame-done callbacks, 20 present pairs, 21 present-detail samples, 20 immediate flushes, 22 FIFO request frames, zero mailbox request frames, and frameDone-to-present avg/max 1.529/6.312 ms.
- [x] FP-5 close-button hover/press visuals are now captured by `lambda-desktop/scripts/verify-frame-pacing-linux.sh`. Latest focused run passed on 2026-06-11 (`.debug-logs/frame-pacing-verify/20260611-204025/chrome-hover-press-cache`): synthetic move, press, move-away, release, and completion events all observed; surface cache stayed hot with 3 hits / 1 miss / zero transient blocks; 11 snap PNGs were captured; close-button metrics reported 6 alpha-valid rows, 4 changing rows, max luma delta 81.964, and luma range 81.963.
- [x] FP-6 native Wayland popover pacing is now enforced by `lambda/scripts/verify-wayland-popover-pacing.sh`. Latest run passed on 2026-06-13 (`.debug-logs/wayland-popover-pacing/20260613-175456`) under headless Weston GL: one immediate initial configure render, eight committed redraw requests, eight frame requests, eight frame-done callbacks, eight committed renders from frame callbacks, zero immediate committed redraw renders, and native popover Escape dismissal through the guarded backend path because Weston headless lacks virtual-keyboard support.
- [x] Manual-gate helper refresh: `lambda-desktop/scripts/run-real-app-smoke.sh` now includes `lambda-editor` and prints the remaining cursor appearance/responsiveness, resize feel, stale-content, and text-after-scroll visual checks. `lambda-desktop/scripts/run-real-app-smoke.sh --probe-only` ran on 2026-06-11: Lambda apps, `foot`, and Firefox are available; GTK app is still missing, and Qt remains optional/missing on this machine.
- [x] Added guarded VT-switch verifier `lambda-desktop/scripts/verify-kms-vt-switch.sh`. It requires an explicit `LWM_VT_SWITCH_TARGET`, refuses to guess a VT, and checks pre/post presentation feedback plus compositor VT release/acquire logs. After the 2026-06-16 repeated-switch crash report, the helper now defaults to two away/back cycles and checks the compositor remains alive during each away and return phase. It was syntax-checked and its non-TTY guard was verified in this Codex session; the actual VT switch still requires a real TTY.
- [ ] Remaining local input/visual gap: `ydotool`, `wtype`, `evemu-event`, and `wayland-info` are installed, real evdev input is validated, and the evdev check is now repeatable via `lambda-desktop/scripts/verify-evemu-hardware-input.sh`. Outside the sandbox the user is now in `wheel` and `/dev/input/event*` is visible with ACLs, but `/dev/uinput` is still `0600 root:root` and `sudo -n true` still requires a password, so uinput-backed validation cannot run non-interactively here. Manual cursor visual validation still requires an interactive human session.
...
- [x] [Auto] Resize a Wayland window continuously with `LAMBDA_RESIZE_TRACE=0`; stderr stays quiet. `lambda-desktop/scripts/verify-wayland-resize-quiet.sh` runs `lambda-terminal` under headless Weston with a terminal-only scripted resize diagnostic and fails if resize tracing, `vulkan-recreate-swapchain`, or swapchain-extent logs leak to stderr. Latest pass: 90 resize steps, zero fatal matches, zero resize-noise matches.
...
- [ ] [Manual] VT switch and mode changes still work. Helper added: `lambda-desktop/scripts/verify-kms-vt-switch.sh` should be run from a real TTY with an explicit `LWM_VT_SWITCH_TARGET`; this Codex session is not a TTY, so it can only verify the script guard.
...
- [x] [Auto] Wayland: `LAMBDA_RESIZE_TRACE=1 app-render` traces show render starting in the same dispatch/flush path as `frameDone`. `lambda-desktop/scripts/verify-wayland-client-pacing.sh` runs `lambda-terminal` under headless Weston GL and asserts frameDone-to-present deltas; latest avg/max 1.340/5.637 ms.
- [x] [Auto] Interactive Wayland resize and animation demos hold refresh rate on a FIFO-only driver (e.g. lavapipe window). `lambda-desktop/scripts/verify-wayland-client-pacing.sh` now forces FIFO with `LAMBDA_VULKAN_FORCE_FIFO_PRESENT_MODE=1`, asserts `mode=fifo` request-frame traces, fails if any mailbox request appears, and keeps frameDone-to-present under the 8 ms gate. Latest pass: `.debug-logs/wayland-client-pacing/20260611-201500`, 22 FIFO request frames, zero mailbox frames, avg/max 1.529/6.312 ms.
...
- [ ] `debug::perf` runtime check — run `lambda-desktop/scripts/verify-macos-metal-editor-perf.sh` on macOS; it drives a large unicode-heavy paste into `lambda-editor`, gates `CanvasDrawableWait` p95/frame-budget p99, and requires `atlasGrow` perf evidence by default.
...
- [ ] Remove this document and TODO-019 from `TODO.md`, and drop the row from `roadmap.md`.
