...
Compositor sources live under `lambda-desktop/apps/lambda-window-manager/Compositor/` in this repository (target `lambda-window-manager`):
...
│   └── protocols/
│       └── wayland/                       # framework-owned protocol XMLs and stubs
├── lambda-desktop/
│   ├── CMakeLists.txt
│   ├── docs/
│   │   ├── compositor.md
...
│               ├── Window/                    # WM facade + FocusStack, PointerRouter, ...
...
├── lambda/src/Platform/Linux/                  # KMS device/output/input
└── lambda-desktop/tests/                       # deterministic compositor unit tests
```
---
