...
usage() {
  cat <<EOF
Usage: lambda-desktop/scripts/run-real-app-smoke.sh [options]
...
lambda_exe() {
  local name="$1"
  local candidates=(
    "$BUILD_DIR/lambda-desktop/$name/$name"
    "$BUILD_DIR/$name"
    "$BUILD_DIR/lambda/demos/$name"
  )
  local path
  ...
  printf "%s/lambda-desktop/%s/%s" "$BUILD_DIR" "$name" "$name"
}
...
compositor_exe() {
  ...
  candidates+=(
    "$BUILD_DIR/lambda-desktop/lambda-window-manager/lambda-window-manager"
  ...
  printf "%s/lambda-desktop/lambda-window-manager/lambda-window-manager" "$BUILD_DIR"
}
...
if case_selected "lambda-editor"; then
  if [[ -x "$(lambda_exe lambda-editor)" || "$BUILD" -eq 1 ]]; then
    add_case "lambda-editor" "required" "lambda-editor" "$(lambda_cmd lambda-editor) $(quote_path "$ROOT/lambda-desktop/docs/TODO.md")" \
