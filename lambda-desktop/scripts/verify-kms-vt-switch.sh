...
SWITCH_CYCLES="${LWM_VT_SWITCH_CYCLES:-2}"
CHECK_TIMEOUT_MS="${LAMBDA_PRESENTATION_FEEDBACK_TIMEOUT_MS:-8000}"

usage() {
  cat <<EOF
Usage: LWM_VT_SWITCH_TARGET=N lambda-desktop/scripts/verify-kms-vt-switch.sh
...
Environment:
  ...
  LWM_VT_SWITCH_CYCLES       Away/back cycles to run. Default: $SWITCH_CYCLES
...
if ! [[ "$SWITCH_CYCLES" =~ ^[0-9]+$ ]] || [[ "$SWITCH_CYCLES" -le 0 ]]; then
  echo "LWM_VT_SWITCH_CYCLES must be a positive integer." >&2
  exit 2
fi
...
fgconsole_current() {
  fgconsole 2>/dev/null ||
    sudo -n fgconsole 2>/dev/null ||
    awk '/^tty[0-9]+$/ { sub(/^tty/, ""); print; exit }' /sys/class/tty/tty0/active 2>/dev/null
}
...
loginctl_activate_vt() {
  local target="$1"
  local session
  session="$(session_for_vt "$target")" || return 1
  loginctl activate "$session"
}

switch_vt() {
  local target="$1"
  chvt "$target" 2>/dev/null || sudo -n chvt "$target" 2>/dev/null || loginctl_activate_vt "$target"
}
...
CURRENT_VT="$(fgconsole_current || true)"
...
cleanup() {
  ...
  if [[ -n "${CURRENT_VT:-}" ]]; then
    switch_vt "$CURRENT_VT" 2>/dev/null || true
  fi
  if [[ -n "$wm_pid" ]]; then
...
printf 'cycles=%s\n' "$SWITCH_CYCLES" >>"$LOG_DIR/vt-switch.log"

for cycle in $(seq 1 "$SWITCH_CYCLES"); do
  printf 'cycle=%s action=away target_vt=%s\n' "$cycle" "$TARGET_VT" >>"$LOG_DIR/vt-switch.log"
  switch_vt "$TARGET_VT"
  sleep "$HOLD_SECONDS"
  if ! kill -0 "$wm_pid" 2>/dev/null; then
    echo "lambda-window-manager exited while on target VT during cycle $cycle." >&2
    tail -n 160 "$LOG_DIR/compositor.log" >&2 || true
    exit 21
  fi
  printf 'cycle=%s action=back current_vt=%s\n' "$cycle" "$CURRENT_VT" >>"$LOG_DIR/vt-switch.log"
  switch_vt "$CURRENT_VT"
  sleep "$HOLD_SECONDS"
  if ! kill -0 "$wm_pid" 2>/dev/null; then
    echo "lambda-window-manager exited after returning to source VT during cycle $cycle." >&2
    tail -n 160 "$LOG_DIR/compositor.log" >&2 || true
    exit 22
  fi
done
...
printf 'SUMMARY kms-vt-switch current_vt=%s target_vt=%s cycles=%s release_events=%s acquire_events=%s before_present=%s after_present=%s fatal_matches=%s log_dir=%s\n' \
  "$CURRENT_VT" "$TARGET_VT" "$SWITCH_CYCLES" "$release_count" "$acquire_count" "$before_present" "$after_present" "$fatal_count" "$LOG_DIR"

if [[ "$fatal_count" -ne 0 || "$release_count" -lt "$SWITCH_CYCLES" || "$acquire_count" -lt "$SWITCH_CYCLES" ||
