...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  cat <<EOF
Usage: lambda-desktop/scripts/trace-terminal-rendering.sh
...
terminal_binary="$BUILD_DIR/lambda-desktop/lambda-terminal/lambda-terminal"
if [[ ! -x "$terminal_binary" ]]; then
  terminal_binary="$BUILD_DIR/lambda-terminal"
fi
if [[ ! -x "$terminal_binary" ]]; then
  echo "lambda-terminal not found in $BUILD_DIR/lambda-desktop/lambda-terminal or $BUILD_DIR" >&2
  exit 1
fi
