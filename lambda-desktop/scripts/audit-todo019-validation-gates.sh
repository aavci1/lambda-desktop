...
usage() {
  cat <<'EOF'
Usage: lambda-desktop/scripts/audit-todo019-validation-gates.sh
...
if [[ -n "$tty_name" && "$tty_name" != "not a tty" ]] && fgconsole >/dev/null 2>&1; then
  gate "READY" "kms-vt-switch" "current_tty=$tty_name; run LWM_VT_SWITCH_TARGET=N lambda-desktop/scripts/verify-kms-vt-switch.sh"
else
...
if [[ -n "${WAYLAND_DISPLAY:-}" || -r "$display_file" ]]; then
  gate "READY" "live-wayland-session" "run lambda-desktop/scripts/run-real-app-smoke.sh for manual visual checks"
else
  gate "PENDING" "live-wayland-session" "start lambda-window-manager, then run lambda-desktop/scripts/run-real-app-smoke.sh"
fi

if [[ "$(uname -s)" == "Darwin" ]]; then
  gate "READY" "macos-metal-runtime" "run lambda-desktop/scripts/verify-macos-metal-editor-perf.sh and compare backdrop blur visuals"
else
  gate "PENDING" "macos-metal-runtime" "requires macOS; current=$(uname -s)"
fi

if [[ -x "$ROOT/lambda-desktop/scripts/verify-evemu-hardware-input.sh" ]]; then
  gate "READY" "real-evdev-helper" "run with LWM_EVEMU_DEVICE=/dev/input/eventN"
else
  gate "PENDING" "real-evdev-helper" "missing lambda-desktop/scripts/verify-evemu-hardware-input.sh"
fi

if [[ -x "$ROOT/lambda-desktop/scripts/verify-kms-vt-switch.sh" ]]; then
  gate "READY" "vt-helper" "lambda-desktop/scripts/verify-kms-vt-switch.sh"
else
  gate "PENDING" "vt-helper" "missing lambda-desktop/scripts/verify-kms-vt-switch.sh"
fi

if [[ -x "$ROOT/lambda-desktop/scripts/verify-macos-metal-editor-perf.sh" ]]; then
  gate "READY" "macos-perf-helper" "lambda-desktop/scripts/verify-macos-metal-editor-perf.sh"
else
  gate "PENDING" "macos-perf-helper" "missing lambda-desktop/scripts/verify-macos-metal-editor-perf.sh"
fi
