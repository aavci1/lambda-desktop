...
usage() {
  cat <<EOF
Usage: scripts/verify-flux-clipboard.sh
...
start_source() {
  ...
  env WAYLAND_DISPLAY="$SOCKET" \
    ...
    "$BUILD_DIR/tools/lambda-clipboard-probe" >"$LOG_DIR/source.stdout" 2>"$LOG_DIR/source.stderr" &
  SOURCE_PID=$!
...
env WAYLAND_DISPLAY="$SOCKET" \
  ...
  "$BUILD_DIR/tools/lambda-clipboard-probe" >"$LOG_DIR/sink.stdout" 2>"$LOG_DIR/sink.stderr" &
SINK_PID=$!
...
env WAYLAND_DISPLAY="$SOCKET" \
  ...
  "$BUILD_DIR/apps/lambda-terminal/lambda-terminal" >"$LOG_DIR/terminal.stdout" 2>"$LOG_DIR/terminal.stderr" &
TERMINAL_PID=$!
