...
- [ ] [Manual] Complete the remaining hardware/visual checks from the plan: manual cursor/hardware input-driver checks, resize/drag visual checks, and representative app inspection requiring interactive input. Current host has `evemu-event`, `wayland-info`, `ydotool`, and `wtype`, but `/dev/uinput` is `root:root` mode `0600`, `ydotoold` cannot open it, and `sudo -n true` still prompts for a password.
