...
- [ ] [Manual] Complete the remaining hardware/visual checks from the plan: manual cursor/hardware input-driver checks, resize/drag visual checks, and representative app inspection requiring interactive input. Current host still lacks `evemu-event`/`evemu` and `wayland-info`/`wayland-utils`; `/dev/uinput` is `root:root` mode `0600`; `sudo -n true` still prompts for a password.
